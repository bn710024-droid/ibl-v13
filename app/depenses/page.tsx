...
export default function DepensesPage() {
  ...
  const sauvegarder = () => {
    ...
    // Pour dépense sync production — on peut modifier le prix ET le volume
    if (editDepense?.isProductionSync) {
      const updated = {
        ...
        depenses: data.depenses.map(d => d.id === editDepense.id
          ? {
              ...d,
              prixParKg: prixKg ? parseFloat(prixKg) : undefined,
              quantiteKg: qteKg ? parseFloat(qteKg) : d.quantiteKg
            }
          : d)
      }
